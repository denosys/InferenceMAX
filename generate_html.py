#!/usr/bin/env python3
# generate_html.py
# Robust, idempotent HTML generator that reads JSON files from data_zips/ and docs/data/,
# writes extracted JSONs into docs/data (keeping existing files), writes data_index.json,
# diagnostics.txt and docs/index.html (Plotly). Requires pandas, plotly.
...
# Configuration (use environment-backed absolute paths)
GITHUB_WORKSPACE = os.environ.get("GITHUB_WORKSPACE", os.getcwd())
INPUT_DIR = os.path.join(GITHUB_WORKSPACE, "data_zips")
OUT_DIR = os.path.join(GITHUB_WORKSPACE, "docs")
...
def save_extracted_jsons(zip_path: str) -> List[str]:
    ...
    for item in extracted:
        ...
        try:
            # If file exists, skip overwrite to preserve existing canonical files
            if out_path.exists():
                # If content differs, write a new version with a run prefix to avoid loss
                try:
                    existing = out_path.read_text(encoding="utf-8")
                    newcontent = json.dumps(obj, ensure_ascii=False, indent=2)
                    if existing.strip() == newcontent.strip():
                        saved.append(out_path.name)
                        continue
                    else:
                        # produce a unique name to avoid clobbering
                        uniq = f"{safe_filename(zname)}__{member_safe}__dup"
                        out_path = Path(DATA_DIR) / (uniq if uniq.endswith(".json") else uniq + ".json")
                        tmp = out_path.with_suffix(out_path.suffix + ".tmp")
                except Exception:
                    pass
...
def build_plotly_html(df: pd.DataFrame) -> str:
    if df.empty:
        ...
        info = (
            ...
            "<h2>No data found in data_zips/ or docs/data/</h2>"
            "<p>The generator did not find JSON data. A sample chart is shown below.</p>"
            "</div>"
        )
...
def main():
    ...
    # Also include any pre-existing files in DATA_DIR
    existing = [p.name for p in Path(DATA_DIR).glob("*.json")]
    all_files = sorted(set(saved_files_all + existing))

    print(f"DEBUG: saved json files: {sorted(saved_files_all)}")
    print(f"DEBUG: existing json files in data dir: {sorted(existing)}")

    try:
        with open(DATA_INDEX, "w", encoding="utf-8") as idxf:
            json.dump({"files": all_files}, idxf, ensure_ascii=False, indent=2)
        print(f"Wrote data index: {DATA_INDEX}")
    except Exception as e:
        print(f"Warning: could not write data index: {e}")

    records = load_all_records_from_saved_jsons(all_files)
    ...
    try:
        with open(diag_path, "w", encoding="utf-8") as d:
            ...
            d.write(f"existing_files: {existing}\n")
